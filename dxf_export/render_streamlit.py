...
def _add_diameter_cotes_for_unique_holes(scene: Scene):
    """Ajoute une cote diamètre AutoCAD par diamètre unique présent dans la scène.

    - Regroupe les Circle par diamètre (tolérance ±0.5 mm).
    - Pour chaque groupe, choisit un cercle représentatif et crée un DiameterDimension
      avec le leader à 45° (haut-droite).
    - Filtre les très petits cercles (rayon < 1 mm — artefacts visuels).
    """
    groups: dict[float, Circle] = {}  # rounded_diam_mm -> representative Circle
    for ent in list(scene.entities):
        ...
        if r < 1.0:
            continue
        diam = r * 2.0
        # Clé arrondie à 0.1 mm pour distinguer correctement les diamètres différents.
        key = round(diam, 1)
    ...
    for _key, circle in sorted(groups.items()):
        cx, cy = circle.center
...
def convert_plotly_figure_to_scene(fig, name="SHEET") -> Scene:
    ...
    _add_diameter_cotes_for_unique_holes(scene)
