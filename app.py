...
def _generate_and_store_exports(all_parts):
    ...
    st.session_state['exports_xls_data'] = create_styled_excel(
        project_info_export,
        pd.DataFrame(all_parts) if all_parts else pd.DataFrame(),
        save_data_export,
    )
...
def _render_exports(all_parts):
    ...
    st.markdown('<div class="kb-step">Etape 4 - Exportation des livrables</div>', unsafe_allow_html=True)
    ...
    already_generated = st.session_state.get('exports_scene_json') == current_scene_json
    scene_changed = (
    ...
    btn_label = "🔄 Regénérer les livrables" if (already_generated or scene_changed) else "⚙️ Générer les livrables"
    generated_now = False
    if st.button(btn_label, type="primary", use_container_width=True):
        with st.spinner("Génération des livrables en cours…"):
            ...
            _generate_and_store_exports(fresh_parts)
            generated_now = True
    ...
    st.markdown(
        ...
        .stDownloadButton > button:hover {
            border-color: #0e2f4f;
            background: linear-gradient(180deg, #f7fbff, #deebf9);
        }
        </style>
        """,
    ...
    with dl_col1:
        if html_ok and html_data:
            st.download_button(
                "📄 Télécharger Dossier Plans (HTML)", html_data,
    ...
    with dl_col2:
        if xls_data:
            st.download_button(
                "📥 Télécharger Fiche de Débit (.xlsx)", xls_data,
    ...
    with dl_col3:
        if dwg_ok and dwg_data:
            st.download_button(
                "📐 Télécharger Plans AutoCAD (.dxf)", dwg_data,
...
def calculate_all_project_parts():
    ...
    for i, cabinet in enumerate(st.session_state['scene_cabinets']):
        ...
        zones_for_parts = calculate_all_zones_2d(cabinet)
...
def _get_expected_password() -> str:
    ...
    secret_pwd = st.secrets.get("access_password") if hasattr(st, "secrets") else None
            ...
            if 'shelves' in cab_render:
                for s in cab_render['shelves']:
                    ...
                    if zone is not None:
                        ...
                        # Regle demandee: etagere fixe en profondeur Y = profondeur caisson - 10 mm.
                        shelf_depth = max(0.0, W - (10.0 * unit_factor)) if s_type == 'fixe' else W
